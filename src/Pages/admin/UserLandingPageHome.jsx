...
const UserLandingPageHome = () => {
  ...
  const getPopularCities = () => {
    axios
      .get(`http://localhost:4000/auth/getnameofcity`)
      .then((res) => {
  ...
  const getAllData = () => {
    axios
      .get(`http://localhost:4000/admin/prioritydata`, {
        headers: { _token: authData.data.token },
      })
      .then((response) => {
  ...
  const handleAddData = () => {
    if (theme !== null && city !== null && title !== null) {
      const url =
        themeId === null
          ? `http://localhost:4000/admin/postpriority`
          : `http://localhost:4000/admin/updateprioritybyid/${themeId}`;
